...
function CartDrawer({ onClose }: { onClose: () => void }) {
  ...
  const toFreeShipping = Math.max(0, 119.99 - total);
        ...
        <div className="px-5 py-3 bg-secondary/60 border-b border-border">
          ...
          <div className="h-1.5 rounded-full bg-border overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(100, (total / 119.99) * 100)}%` }}
              role="progressbar"
              aria-valuenow={Math.round((total / 119.99) * 100)}
              aria-valuemin={0}
              aria-valuemax={100}
            />
          </div>
  ...
  return (
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
      ...
      <div className="bg-primary text-primary-foreground text-center text-xs py-2 px-4 font-medium tracking-wide">
        🔥 <strong className="font-bold">COMPRE JUNTO</strong> · 3 camisetas por R$&nbsp;119,99 · Frete grátis acima de R$&nbsp;119,99
      </div>
